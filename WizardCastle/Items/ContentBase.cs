...
namespace WizardCastle {
    class ContentBase : ItemBase {
        private static Action<State> Wrapper(string name, Action<State> onEntry) =>
            (state) => {
                Util.WriteLine($"\nHere you find '{name}'");
                onEntry?.Invoke(state);
            };

        public ContentBase(string name, Action<State> onEntry = null) : base(name, ItemType.Content, Wrapper(name, onEntry)) { }
